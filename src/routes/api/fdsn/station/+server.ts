...
function resolveTargetUrl(requestUrl: URL): string {
    ...
    return `${baseUrl}/fdsnws/station/1/query?${forwardParams.toString()}`;
}
...
export const GET: RequestHandler = async ({ url }) => {
    ...
    try {
        const response = await serverFdsnFetch(targetUrl, {
            headers: {
                'User-Agent': 'EWS-Concept/1.0 (FDSN Station Proxy)',
        ...
        console.error(`[FDSN Station Proxy Error] ${targetUrl}:`, error);
        return json(
            { error: 'Gagal mengambil data stasiun dari upstream', details: error.message },
