...
<div
  class="magi-rack-container flex flex-col gap-2 p-4 bg-[#ff4e00] rounded-md border border-orange-700 shadow-xl select-none {className}"
>
  <div
    class="flex items-center justify-between pb-2 border-b border-black/25 text-xs font-bold text-black"
  >
    <div class="flex items-center gap-2">
      <span class="w-2 h-2 rounded-full bg-black animate-pulse"></span>
      <span>MAGI DATA RACK // TELEMETRY CHANNELS</span>
    </div>
    <span class="text-[10px] bg-black text-emerald-400 px-2 py-0.5 rounded"
      >ONLINE</span
    >
  </div>
      ...
      >
        <div class="flex flex-col">
          <span class="font-bold text-xs text-black"
            >{rackItem.channelName ?? rackItem.id}</span
          >
          <span class="text-[10px] text-black/70"
            >{rackItem.category ?? "FEED"}</span
          >
        </div>
        ...
        <div class="w-36 h-8">
          <svg class="w-full h-full" viewBox="0 0 140 32">
            ...
            <path
              d={getWirePath(
                { x: 5, y: 16 },
                { x: rackItem.connected ? 35 : 35 - shift, y: 16 },
                !rackItem.connected,
                5,
              )}
              fill="none"
            ...
            <path
              d={getWirePath(
                { x: rackItem.connected ? 105 : 105 + shift, y: 16 },
                { x: 135, y: 16 },
                !rackItem.connected,
                5,
              )}
            ...
            <g transform="translate(70, 16)">
              <path
                ...
                style="transform: translate({!rackItem.connected
                  ? -shift
                  : 0}px, 0);"
